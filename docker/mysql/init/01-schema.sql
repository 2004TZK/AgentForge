...
CREATE TABLE IF NOT EXISTS `document` (
  ...
  `file_type`       VARCHAR(50)     NOT NULL                COMMENT 'pdf/docx/txt/md/db/sqlite/sqlite3/csv',
  `status`          VARCHAR(20)     NOT NULL DEFAULT 'PENDING' COMMENT 'PENDING/PROCESSING/READY/FAILED',
  `chunk_count`     INT UNSIGNED    NOT NULL DEFAULT 0      COMMENT '切片数（入库完成后回填）',
  `slicing_mode`    VARCHAR(16)     NOT NULL DEFAULT 'auto' COMMENT '切片方式：auto/manual',
  `slicing_config`  JSON            NULL                    COMMENT '手动切片参数快照（重试沿用）',
  `processed_chunks` INT UNSIGNED   NOT NULL DEFAULT 0      COMMENT '已入库 chunk 数（进度回写）',
  `total_chunks`    INT UNSIGNED    NOT NULL DEFAULT 0      COMMENT '总 chunk 数（解析完成后回写）',
  `deleted`         TINYINT(1)      NOT NULL DEFAULT 0      COMMENT '逻辑删除',
  `created_time`    DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_time`    DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
