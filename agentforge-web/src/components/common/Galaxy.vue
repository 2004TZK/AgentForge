...
/**
 * Galaxy 星空背景（WebGL/OGL 全屏 shader 星海）。
 * 基于开源 Gooey 星系效果适配 AgentForge 星空主题：
 * - 星星粒子色相固定为星辉金（uHueShift 默认 42°≈ #ffcf6b），饱和度带轻微随机
 * - 鼠标交互监听挂到 window（背景层 pointer-events: none，避免遮挡页面操作）
 * - 透明背景，可置于任何页面之下作为全局星海
 */
...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // 星辉金主色：uHueShift 为绝对色相（默认 42°≈ #ffcf6b），
      // 每颗星加 ±~0.0175 的色相抖动，饱和度按星随机（亮金主体 + 少量近白星光）
      float hue = fract(uHueShift / 360.0 + (seed - 0.5) * 0.035);
      float sat = (0.35 + 0.65 * seed) * uSaturation;
...
const setup = () => {
  ...
  // 背景层 pointer-events: none，鼠标监听挂到 window（全屏跟随）
...
<template>
  <div ref="ctnDom" class="galaxy-bg" v-bind="$attrs" />
</template>
